...
const priority = global.getMessagePriority(chatUpdate);
const chatId = chatUpdate.messages[0]?.key?.remoteJid;

if (chatId) {
    if (priority >= 3) {
        if (!global.priorityQueue.has(chatId)) {
            global.priorityQueue.set(chatId, []);
        }
        global.priorityQueue.get(chatId).push(chatUpdate);
        return;
    }
    
    if (!global.messageQueue.has(chatId)) {
        global.messageQueue.set(chatId, []);
    }
    global.messageQueue.get(chatId).push(chatUpdate);
    return;
}
